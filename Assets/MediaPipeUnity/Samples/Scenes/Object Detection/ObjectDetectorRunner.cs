...
namespace cup
{
    ...
    {
        public static Vector2 cupPosition = Vector2.zero;
    }
}
        ...
        private void OnObjectDetectionsOutput(ObjectDetectionResult result, Image image, long timestamp)
        {
            ...
            if (result.detections.Count > 0)
            {
                for (int i = 0; i < result.detections.Count; i++)
                {
                    ...
                    var label = detection.categories[0].categoryName;

                    if (label == "cup") 
                    {
                        
                        float centerX = (detection.boundingBox.left + detection.boundingBox.right) / 2;
                        float centerY = (detection.boundingBox.top + detection.boundingBox.bottom) / 2;

                        cup.cupdata.cupPosition = new Vector2(centerX, centerY);  
                        Debug.Log($"Cup detected at position: {cup.cupdata.cupPosition}");
                    }
